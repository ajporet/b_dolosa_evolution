...
min_average_coverage_to_include_sample = 12; 

%Filter on a per-call basis
min_maf_for_call = .9; % maf = major allele frequency
min_cov_per_strand_for_call = 12;% cov = coverage
...
min_median_coverage_position = 40; % note: related to min_average_coverage_to_include_sample
promotersize=250;

%filters for using once we have a trusted position
min_maf_for_analysis=.80;

include_dirty_sample_in_tree = 10; %maximum number of N's in a sample after all other filtering 
...
Calls=maNT;
Calls(Quals < min_qual_for_call | maf< min_maf_for_call | coverage_forward_strand < min_cov_per_strand_for_call | coverage_reverse_strand < min_cov_per_strand_for_call)=0; %Quals < min_qual_for_call |
...
% select = find(ismember(gene_nums, pot_recomb_genes));
% samp_nums = find(sum(hasmutation(find(ismember(gene_nums, pot_recomb_genes)),:),1)>0);
% 
% clickable_snp_table(annotation_full(select), Calls(sitetoparse(select),samples_to_show(samp_nums)), counts(:,sitetoparse(select),samples_to_show(samp_nums)), SampleNames(samples_to_show(samp_nums)), ScafNames, MutQual(positions_to_show_in_table(select)), QualSort);
...
ChrStarts_tmp = [ChrStarts,GenomeLength];
for j=1:numel(goodpos)
    i = goodpos(j);
    chromo = max(find(p(i)>ChrStarts));
    same_chr = find((p>ChrStarts_tmp(chromo)) & (p<ChrStarts_tmp(chromo+1)));

    %find nearby snps
    region_tmp=find(p>p(i)-recombination_block_size,1):find(p<p(i)+recombination_block_size,1,'last');
    region = intersect(region_tmp,same_chr)';

    if numel(region)>=2
        %for each pair in here, if high enough covariance (relative to variance), discard
        region_for_cov = fixedmutation(region,:)';
        data_size = size(region_for_cov,1);
        col_num = size(region_for_cov,2);
        
        max_possible_cov_mat = zeros(col_num,col_num);
        for m=1:col_num
            for n=1:col_num
                reg_to_sim = region_for_cov(:,[m,n]);
                num_muts = sum(reg_to_sim);
                max_possible_cov = min(num_muts)./data_size  - (num_muts(1)*num_muts(2)./(data_size*(data_size-1)));
                max_possible_cov_mat(m,n) = max_possible_cov;
            end
        end

        covariancematrix=cov(fixedmutation(region,:)');
        all_cov = [all_cov,covariancematrix(:)'];
        all_cov_max = [all_cov_max,max_possible_cov_mat(:)'];
        %[a,b]=find(covariancematrix>covariance_cutoff*repmat(max(covariancematrix,[],2),1,length(covariancematrix)));
        %[a,b]=find(covariancematrix>covariance_cutoff.*ones(size(covariancematrix)));
        [a,b]=find(covariancematrix>max_possible_cov_mat*covariance_cutoff);

        involved_in_non_snp_event=[involved_in_non_snp_event region(a(a~=b))];
    end
end
involved_in_non_snp_event=unique(involved_in_non_snp_event);
involved_in_non_snp_event_filter_passing = ismember(positions_to_show_in_table, involved_in_non_snp_event);

removed_SNPs = annotation_full(ismember(positions_to_show_in_table,intersect(positions_to_show_in_table, involved_in_non_snp_event)));

%affected_samples = sum(hasmutation(ismember(positions_to_show_in_table,intersect(positions_to_show_in_table, involved_in_non_snp_event)),:));
%affected_samplenames = SampleNames(samplestoplot(ismember(samplestoplot,find(affected_samples>0))));

poor_muts = hasmutation(involved_in_non_snp_event_filter_passing,:);

affected_samples = cell(size(poor_muts,1),1);
for i=1:size(poor_muts,1)
    affected_samples_tmp= poor_muts(i,:);
    disp(sum(affected_samples_tmp))
    affected_samples(i) = {SampleNames(affected_samples_tmp>0)};

end

%%%%%%%%%
new_goodpos = goodpos(~ismember(goodpos,involved_in_non_snp_event));
% annotations_all_recomb = annotate_mutations_gb(p2chrpos(p(involved_in_non_snp_event),ChrStarts),REFGENOMEFOLDER) ;
% annotations_all_recomb_full= append_annotations(annotations_all_recomb, ancnti(involved_in_non_snp_event), Calls(involved_in_non_snp_event,:), counts(:,involved_in_non_snp_event,:), hasmutation(involved_in_non_snp_event,:), promotersize) ; %% adds information about particular mutations observed, based on
% 
% 
% recomb_pos_pre_filt = [annotations_all_recomb.scaffold;annotations_all_recomb.pos]';
% removed_samples = SampleNames(sum(fixedmutation(involved_in_non_snp_event,:),1)>0);
% removed_samples_lps = SampleNames(sum(maf(involved_in_non_snp_event(8:10),:)<1)>0);
% 
% recomb_pos_pre_filt = [annotations_all_recomb.scaffold;annotations_all_recomb.pos]';
% originally_filtered_out = zeros(length(recomb_pos_pre_filt),1);
% originally_filtered_out(ib)=1;
% 
% [C,ia,ib] = intersect(positions_to_show_in_table,involved_in_non_snp_event,'legacy');
...
tempnames = generate_phylip_input(calls_for_tree, TreeSampleNames, ['2024-11-13-21-01-58_out_infile.txt']);
