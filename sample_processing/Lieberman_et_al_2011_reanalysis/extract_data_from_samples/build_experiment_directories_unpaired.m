...
elseif all([SampleTable.Lane]<0)
    ...
    disp(SampleTable(1))
    cmds = moverenamefastqs_single_read(SampleTable);
...
for i=1:length(SampleTable)
    ...
    for f=fi(:)'
        ...
        % The output for cutadapt is set to be cutadapt_reads_1.fastq to
        % avoid the discrepancies of having to change every other name in
        % this file
    ...
    for a = s.Alignments'
        ...
        if ~exist([dr '/aligned.sam']) & ~exist([dr '/aligned.sorted.bam'])
            ...
            switch ae.Method
                ...
                case 'bowtie2' %USE THIS!!!!!!!!
                    if Phred_offset == 64
                        %cmds{end+1} = ['bowtie2 --phred64 -x genome_bowtie2' ...
                        %    ' -U   ' drf '/filter_reads_1.fastq -S ' dra '/aligned.sam --un ' dra '/unaligned.fastq '];
                        cmds{end+1} = ['bowtie2 --phred64 --sensitive --n-ceil 0,0.01 -x genome_bowtie2' ...
                            ' -U   ' drf '/filter_reads_1.fastq -S ' dra '/aligned.sam --un ' dra '/unaligned.fastq '];
                    else
                        %cmds{end+1} = ['bowtie2 -x genome_bowtie2' ...
                            %' -U   ' drf '/filter_reads_1.fastq -S ' dra '/aligned.sam --un ' dra '/unaligned.fastq '];
                        cmds{end+1} = ['bowtie2 --sensitive --n-ceil 0,0.01 -x genome_bowtie2' ...
                            ' -U   ' drf '/filter_reads_1.fastq -S ' dra '/aligned.sam --un ' dra '/unaligned.fastq '];
                    end
